        ...
        if implementation.startswith('native'):
            if family == 'settle':
                self.compiled, self.engine, self.encoded_q = native(self.spec, m, self.q, mode='hst',
                    full_autograd=full_autograd, aggregate_autograd=aggregate_autograd, packed=implementation != 'native-serial', workers=1 if implementation == 'native-serial' else 3)
                ...
                self.engine = Native(self.graph, m, algorithm=algorithm, mode='hst', full_autograd=full_autograd, aggregate_autograd=aggregate_autograd,
...
def trajectory(dtype, family, implementation, memory, kind, native_optimizer=False, full_autograd="replay", aggregate_autograd="replay"):
    c = TrainingCase(dtype, family, implementation, memory, full_autograd, aggregate_autograd)
