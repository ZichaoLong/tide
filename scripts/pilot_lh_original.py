...
def main():
    ...
    p.add_argument('--reuse-smoke-prepared', help='reuse an audited completed smoke build, never a run directory')
    args = p.parse_args()
    ...
    def measure(label, mode, small=False):
        prepared = args.reuse_smoke_prepared if small and args.reuse_smoke_prepared else str(out / (label + '-prepared'))
        return run(label + '-' + mode, [sys.executable, 'scripts/benchmark_lh_original.py',
            '--device', 'cpu', '--dtype', 'float32', '--prepared', prepared,
    ...
    if not args.reuse_smoke_prepared and not build('smoke', 'wide', ['--width', '64', '--batch', '4', '--steps', '4']):
        return 1
