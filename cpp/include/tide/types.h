...
struct SourceDomain { std::vector<Index> edge_target, input; };
struct Graph {
  ...
  std::optional<SourceDomain> source_domain;
  std::vector<Index> source_counts;  // Logical incoming domain size per node.
  std::string identity;
  void compile();
