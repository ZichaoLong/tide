...
空间图拓扑在执行中固定、单节点成本不随总容量增长、可达总容量继续增长
    ↓
固定拓扑本身不代表度有界；
但每条直接连接都会占用接口、状态、候选处理或通信资源，
所以单节点成本有统一上界时，单节点连接度也不能随总容量一直增长
    ↓
...
1. **Flat MoE 是已经得到正面验证的起点。** 它证明了很大的潜在参数量可以和每 Token 少量昂贵计算同时成立。它的拓扑也可以是固定的，但通常把一个随规模增长的专家池作为一步可选的全局候选集合，并在选择后迅速 merge。它没有验证的是：单节点成本保持有界时，模型能否通过固定空间图上的多跳传播利用持续增长的容量。模块私有状态是后文为解决分支历史问题加入的独立假设，不是这一步直接推出的要求。

2. **固定空间拓扑、单节点成本有界和总容量增长，共同要求多跳扩展。** 固定拓扑只说明执行期间不临时改边，本身不限制一个节点有多少邻居。但 Tide 的成本口径还要求每个节点持有的参数和状态、连接接口、候选处理与通信成本都不随总容量增长。只要每条直接连接都会占用其中某种资源，单节点的入度和出度就不能一直增长。在入口数量也有上界时，固定跳数能够到达的节点数不会随总容量增长；要让更多容量保持可达，传播跳数或空间范围就必须随规模扩大。本文把这个方向称为“有界度的多跳扩展”。规则层次递归不是唯一形式，但它规则、容易从 checkpoint 生长，也适合作为进入 line、lattice、mesh、多尺度 backbone 和一般局部 DAG 之前的工程起点。
...
因此，“有界度的多跳扩展”来自固定空间拓扑、单节点成本上界和可达容量增长这三个要求；“多次局部选择和总预算”则进一步来自每 Token 的稀疏计算要求。它们共同给出一个结构方向，不代表某种具体架构已经验证成功。
