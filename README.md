...
- 一个固定身份的 256-slot、固定 K `core-v1` executor candidate corpus，其中 64 个预标记 VJP；通用 packed 的静态支持谓词接受 256/256，单层与 HB 特化谓词分别预选 8/16 个。开发 suite 对全部 256 个 Plan 在 FP64/FP32 下比较 eager—packed 的 output、state、balance、full trace、route invariant，并直接比较整段 prefill、两段 chunk 和逐 Token decode 的 forward；每段 chunk 和每步 decode 的 trace 也直接对照 eager，并在规范合并后与整段 trace 对照。每个预标记 VJP case 在两种 dtype 下从同一次保留计算图依次隔离查询 output、可微 balance loss、每个最终状态 owner/component、每个 region 的 `soft_sum`、每个可微 trace region event 的 logits/probabilities、组合目标和再次查询的 output，并逐项比较 hidden、全部具名参数及其 `None` 连通性。另有 FP64 定向回归覆盖 EMA、Gated DeltaNet、窗口 Attention 的可微外部初态（Attention 分开检查 keys/values），以及保留计算图跨两个 chunks 的 public-result objectives；固定反例还逐个检查一个多候选 Plan 的 node-event logit root，防止 eager 的整组 logits 堆叠与 packed 分组执行产生 connected-zero/`None` 偏差。对全部 24 个静态适用 case，suite 还执行 eager—packed—specialized 三方 FP64/FP32 full/chunk/decode forward 与另行 full-prefill VJP，并用有状态代表集覆盖 reset、row reorder 和 empty tail；这些 candidates 尚未物化为资格计划要求的冻结 bundles；
...
- `tide.generic-packed.torch.v1`：直接使用同一 `SettleGraph` owner 的参数，对当前任意固定 K `core-v1` DAG 按 region 处理整段 \([B,T]\)，在 regions 间只保留实际 `DATA` records；覆盖当前三种 state、六种 profile/timing、prefill/decode、forced singleton、trace 和梯度路径，不调用 eager scheduler。SD/pre 的离散 route discovery 和因果 state replay 放在 TorchScript 循环中；
...
region-major reference 仍按 region、Token 和 batch row 使用 Python 循环，只是 eager 调度 oracle。新的 packed 路径已经 tensorized，但当前实现仍有 StateStore 打包/发布、静态参数分组等 host-side mapping，严格 FP32 顺序的部分递推位于编译循环；没有真实 profiler、长序列 benchmark、峰值内存和 NPU fallback closure 前，不能把它记为通过性能资格。尚未完成的主要范围包括：
