...
| **ESN-003** | P0 | 已修改 | 2.3、2.4、3.3、3.4、附录 A.2、6.4、7.1 | `SEL-CONTENT` 被定义为只读当前消息，但历史负载等持久信息也可能影响 selector。 | 不设置集中保存完整 receiver 状态的 SelectorState；允许按 region/node 保存轻量独立 history，或并入 receiver state，并在 manifest 标明。 |
| **ESN-004** | P0 | 已修改 | 附录 A.5 | Attention state 的示例默认 receiver 保存窗口内每个 Token，但 SD 只能保存该 receiver 实际 Observe 的 Token；当前公式实际只自然对应 BO。key 维度 \(K\) 与 key 矩阵 \(K_{\ell,t}\) 也发生重名，堆叠的 key/value 矩阵未定义。 | 用“实际 Observe 的时间集合”定义历史；明确 Append/Evict 的 `Update`；定义堆叠矩阵并用 \(d_k\) 等符号表示维度；说明完整历史是否只在固定最大上下文下视为有界。 |
...
| **ESN-010** | P1 | 关闭 | 2.6、附录 A、8 | 公式从 \(S_{t-1}\) 开始，但没有定义序列首状态；也没有在核心语义中明确状态逐序列隔离、无效 Token 是否 Update、chunk 是继承还是清零、跨 chunk 是否截断梯度。`prefill = decode` 也未作通俗解释。 | 已定义空首状态、有效 Token 规则、逐序列隔离、跨 chunk carry 与默认 detach，并明确 `prefill = decode` 的判定。 |
...
| **ESN-013** | P2 | 关闭 | 文首、2、3、附录 A、6.3 | 若文档面向可独立阅读的领导或新读者，TIDE、N、SD、BO、M8、SSM、SSD、ST-MoE、`noaux_tc` 等首次出现时仍缺少展开或一句解释。EMA 的 \(\lambda_i\) 是标量还是向量、GDN 的 q/k/value 维度也被省略。 | 首次出现时补最短定义；补 \(\lambda_i\) 的取值范围/形状和 GDN 核心张量维度，不扩写成综述。 |
| **ESN-014** | P2 | 已修改 | 附录 A.4、A.6、6.3 备注 | 外部模型事实基本正确，但负载均衡表缺少官方出处；“公认有效”偏强，“KDA 是 GDN 的近期改进”也容易被理解为严格继承关系。 | 为模型/报告名加入官方链接；把 z-loss 改成“常用的可选稳定项”；把 KDA 表述为 delta-rule 家族中采用更细粒度门控的后续路线。 |
...
| **ESN-016** | P0 | 已修改 | 2.3、3.1、3.4、3.5、附录 A、4.4、7.2、8 | receiver state 只条件化 FFN 输入，无法表达状态/Attention residual 后再接 Pre-Norm FFN 的默认节点模板。 | `Read^ffn` 统一返回 hidden residual；默认模板依次执行状态/上下文 residual 与 FFN residual；N 令该读出为零；两个子层合计仍算一个 H 层级。 |
| **ESN-017** | P0 | 关闭 | 2.3、2.4、3.1—3.4、附录 A、7.1、8 | group 公共入口 norm 让所有 receivers 共享同一个可学习输入适配器，也混合了 selector 公共输入与 receiver 本地消息两种角色。 | selector 使用独立 `N_sel`；每个 receiver node 使用自己的 `N_R,i`，只向 selector 发送轻量 `Read^sel`；RMS 统计可复用，但可学习 scale 不共享。 |
...
| 2026-08-25 | ESN-008 | 保留简短兼容表：N 仅 content，SD 支持 content/pre，BO 支持三者；broadcast-proposal 只保留为帮助发散的说明。 | 工作区修改（待复核） |
...
| 2026-08-26 | ESN-002、015 | GraphBranch 内部边统一传递完整 hidden；只有 GraphBranch 与 backbone 的边界使用 RESIDUAL_ADD。具体 H2 topology 仍待对齐。 | 本次提交（部分） |
| 2026-08-26 | ESN-002、007、015 | receiver nodes 只产生完整候选；Soft-P、Hard-ST、Top-K 和 RESIDUAL_ADD 统一由 EmitPolicy 表达，router 概率不再被重复使用；GATE 名称字段改为 AGG。 | 本次提交 |
...
| 2026-08-28 | ESN-002、005、007 | 增加 reached set、region selector、一次性多父 `MessageAggregate` 和波前 barrier；HB balance loss 与 selector 概率进入主任务梯度的位置继续保留为待对齐项。 | 本次提交（部分） |
| 2026-08-28 | ESN-001、005、007、015 | HB selector 概率在 sender 的 delta Hard-ST `EmitPolicy` 进入主任务梯度；多父 `MessageAggregate` 独立使用均值；region balance 采用 availability-conditioned soft 目标，并单列诊断量与命名字段。 | 本次提交（待逐项核验） |
...
| 2026-08-29 | ESN-001、002、005、007、015 | 本轮审校修正请求激活数与实际激活数的区分、聚合公式适用范围、selector 历史负载措辞、状态/缓存隔离、Plan 边界检查和命名解析说明；不改变已对齐的拓扑语义。 | 工作区修改（待复核） |
| 2026-08-29 | ESN-002、005、007、012、015 | 继续审校：核心 profile 按 GraphBranch/site 统一并由 node/region 继承；Score 只对 reached candidate 定义；补充单层 Top-K 的 hard active share；负载历史扩展改用 SEL-CUSTOM；补充 post-update + BO 的 commit 步骤、AGG-MEAN 权重和 Plan 节点可达性检查。 | 工作区修改（待复核） |
| 2026-08-29 | ESN-002、015 | 同步 README 与 `experiment-protocol.md` 的 `PropagationProfile` 清单，补上 N（无状态）选项。 | 工作区修改（待复核） |
| 2026-08-29 | ESN-001、002、019、021 | 按陌生读者顺序复核第 2—4 节：基础节先定义 (h,p) 等共用符号和完整执行顺序；补齐单层状态/K 记法、HB 的逐 Token barrier、目标缓存键、多父实际到达消息和动态输出路径说明。 | 工作区修改（待复核） |
| 2026-08-29 | ESN-001、002、005、007、015 | 继续全面审校：固定槽位拼接改用 \(F,\widetilde g\)；明确 proposal→selector 的主任务梯度由 EmitPolicy 决定；修正 RMSNorm 公式、K 摘要、Plan 首次出现、HB 端点符号和动态可达路径表述；补充 query \(q^{\mathrm{qry}}\) 与 reached 标记的区分。 | 工作区修改（待复核） |
...
| **OK-004** | 第 2.4 节的 EMIT-HST 前向完整保留 active receiver 输出，梯度通过 selector 概率返回；离散 Top-1 / Top-K 本身不反传；`MessageAggregate` 不重复使用该概率。 |
| **OK-005** | 第 6.1、6.3 节的单层 receiver balance loss、M8 Switch-style balance loss、stop-gradient 和 router z-loss 已在规范中分别定义；历史代码差异以对应 commit 和 manifest 核对。 |
...
| **OK-010** | 第 2.3、2.4 节与附录 A 的 `N_sel`、receiver-local `N_R,i`、三种 `Read^sel` 时序、状态样例和计算量说明使用同一套独立入口语义。 |
...
2026-08-29：全面通读第 1—4 节、基线、损失、命名、必填清单和附录。已修正复合状态的时间快照（\(s^-\) 为 Token 前状态、\(\widetilde s\) 为 proposal、\(s^{\mathrm{cmp}}\) 为当前计算可见的 commit 后状态、\(s\) 为 Token 末最终状态），并同步执行顺序、单层例子与附录；同时修正入口导航、HB 延迟与 selector-history 依赖、下游端口措辞和 K/STATE 命名说明。未发现新的 P0；ESN-002 及 HB-001—006 的拓扑细节仍按原台帐状态待后续核验。

2026-08-30：继续按陌生读者路径复核第 2 节及其与第 3、4 节的衔接；确认共用 \(h,p\) 符号、节点执行步和跨 Line 规则已在基础节定义，并把 forced-active 收窄为 singleton region，混合 region 明确归入 CUSTOM。未发现新的 P0；ESN-002 及 HB-001—006 仍待逐项核验。
