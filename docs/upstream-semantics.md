...
| 不可变提交 | `e529de212c605f7f417c3a1ce97e780a2ee59824` |
| 语义版本 | `tide-core-3` |
| 语义锚点路径 | `20-tide-decentralized-neural-network/semantics-anchor.md` |
| 锚点原始文件 SHA-256 | `627a0603c58bbf69051b88b7a3fafaca9aad1ff4247b2a8516bb2ad5460cc341` |
| 采用的模型族 | SettleGraph；通过上游声明的逻辑日程与投影，对应 TimedDAG 的受限实例 |

这些值保存在 [tide-core.lock.json](upstream/tide-core.lock.json)。哈希针对该提交下锚点的原始文件字节，不能用网页渲染内容计算。Plan 的 canonical bytes 不因重新发布语义权威而改变；trace、fixture、checkpoint 和 development run 在 Plan 外层保存机器可读的语义权威、逻辑日程及解释上下文。

本次采用的教材均固定在同一提交：

- [语义锚点与仓库分工](https://github.com/ZichaoLong/ObsidianVault/blob/e529de212c605f7f417c3a1ce97e780a2ee59824/20-tide-decentralized-neural-network/semantics-anchor.md)
- [SettleGraph 教材](https://github.com/ZichaoLong/ObsidianVault/blob/e529de212c605f7f417c3a1ce97e780a2ee59824/20-tide-decentralized-neural-network/settlegraph-learning-note.md)
- [TimedDAG 与区域选择](https://github.com/ZichaoLong/ObsidianVault/blob/e529de212c605f7f417c3a1ce97e780a2ee59824/20-tide-decentralized-neural-network/timed-dag-region-selector-learning-note.md)
- [TimedDAG 的节点级时间批](https://github.com/ZichaoLong/ObsidianVault/blob/e529de212c605f7f417c3a1ce97e780a2ee59824/20-tide-decentralized-neural-network/timed-dag-chunk-prefill-learning-note.md)
- [正时延 Graph 与有限切面](https://github.com/ZichaoLong/ObsidianVault/blob/e529de212c605f7f417c3a1ce97e780a2ee59824/20-tide-decentralized-neural-network/positive-delay-graph-finite-cut-learning-note.md)
...
| Aggregate、入口归一化 | 从完整输入得到本地内容的纯函数；Agg 接收按固定顺序排列的身份—值序列，可以读取入口／父边标签，或作为简单实例忽略标签只读数值 |
...
| Observe 与状态采用 | 确定本次计算快照；它与下一持久状态是两个角色 |
...
| 终端消息聚合与 Base placement | 输出 Agg 接收按 node ID 排列的终端身份—值序列；具体张量公式与模型组合由本地定义 |
...
| 只读取值的 Aggregate、同一 hidden 复制到全部出边 | 上游 SettleGraph 接口中忽略身份标签的受限选择 | 完整公式、规范顺序及广播范围 |
| 读取父 edge ID 或终端 node ID 的 Aggregate | 上游 SettleGraph 的标准实例；身份标签由核心输入接口保留 | 逐公式登记所读身份、缺失消息规则和参数成本；若 Agg 含主要计算，还须调整成本 profile 并提供对应 batch witness |
...
父边感知 receiver 聚合和终端节点感知输出聚合不再登记为本地扩展；它们与忽略身份的均值一样，都是 tide-core-3 的 SettleGraph 实例。输入提供 K、超出已声明控制量的额外读入等其余本地扩展仍不修改上游接口，也不能借用不满足前提的上游定理。每个 Plan、fixture 和实验仍须说明具体公式、成本 profile、较窄限制与真正的本地扩展；本地 `core-v1` 的资格名称本身不构成全面符合性标签。以后再增加变化时，应列出改变的函数、自变量、状态所有权、时间或观察契约，再判定它是合法实例、较窄限制，还是需要另立版本的语义扩展。`CUSTOM` 名称或通过一个本地 validator，都不能替代这个判断。

## 几种版本各指什么

| 名称 | 版本对象 | 与其他版本的关系 |
| --- | --- | --- |
| `tide-core-3` | 上游抽象接口、依赖边界和教材 | 本页固定的共同语义；不是软件完成度 |
| 本地 `core-v1` | 已声明的实现与资格子集 | 保留其固定 K、配置及测试范围；其中身份感知聚合现属于 tide-core-3 标准实例 |
| 本地 `extension-v2` | 本地扩展接口和资格进入范围 | 不是上游第二版；其中的固定参数共享可仍是 tide-core-3 的合法实例 |
...
[首次采用 tide-core-2 前的实验语义备份](archive/experiment-semantics-and-naming-pre-tide-core-2.md)仅用于解释历史材料，**不是现行规范**。当前入口见[文档导航](README.md)，当前具体实验定义见[实验语义文档](experiment-semantics-and-naming.md)。
