...
| M7 | LH inference adapter using original C++; exact clock/readout/decay mapping; numerical qualification without changes to LH | [Selector](evidence/lh-selector.md), [Add](evidence/lh-add.md), [Full](evidence/lh-full.md), [same-fiber sum attention baseline](evidence/lh-attention.md) verified; packed/weighted attention, readout and full model pending |
...
[qualified](evidence/lh-attention.md) (`fiber-attention.md`). Next packed/weighted
attention gates: `lh-attention-plan.md`; remaining whole-model
