...
| M7 | LH inference adapter using original C++; exact clock/readout/decay mapping; numerical qualification without changes to LH | [Selector](evidence/lh-selector.md), [Add](evidence/lh-add.md), [Full](evidence/lh-full.md) components verified; attention/readout/full model pending |
...
(`lh-full.md`). Next attention gate: `lh-attention-plan.md`; remaining whole-model
